"""TPC-DS v3.2 table definitions for Bronze layer.

All Bronze columns are loaded as TEXT to preserve source fidelity;
Silver handles type casting and business-rule cleansing.
Column counts verified against dbgen 2.10.0 SF1 output.
"""

# table_name -> (source_filename, [column_names])
...
def bronze_ddl(table: str) -> str:
    ...
    return f"CREATE TABLE IF NOT EXISTS bronze.{table} (\n    {col_defs}\n);"


# Dimension tables with a single-column surrogate key. Used to generate
# dbt source-level not_null + unique tests. Facts (catalog_sales,
# catalog_returns, store_sales, store_returns, web_sales, web_returns,
# inventory) have composite or no PK in Bronze and are omitted.
